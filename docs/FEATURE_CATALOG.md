...
| DFM analysis (Toolpath) **[module]** | 🛠 (👤?) | Send an uploaded CAD part to Toolpath's API, get design-for-manufacturability findings back, attach them to the part revision and surface them in the estimate. **Requires Part viewing** — it reuses the same file and the same async-derivation pattern (upload → poll a Job; there are no webhooks). API-key auth, OpenAPI spec at `/v1/openapi.json`. **Export-controlled files must never be sent** — the gate is structural, see `docs/MODULES.md` § Toolpath. Whether customers see DFM output is an explicit decision, not a default. |
