...
Google Workspace · Microsoft 365 / Outlook · Payments · Transactional email
...
design-for-manufacturability information. Resources cover Parts,
Features, Holders, and Jobs; auth is an **API key** (company scope, not
OAuth); there's an OpenAPI spec at `GET /v1/openapi.json` to generate a
client from — pin the spec version you generated against.
...
**There are no webhooks** — you upload and poll a Job. That means a
background job with backoff and a give-up bound, not a request handler
...
Already in the catalog (Online payments, Transactional email). Payments
is webhook-driven — `secure_coding.md` § 16 governs it. Email is closer
to infrastructure than to a module; the bare minimum is auth email.
