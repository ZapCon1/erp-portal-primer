...
**Version 0.22.2** (2026-09-05) — see `CHANGELOG.md`. Record this version in
...
| `docs/modules/` | `docs/` | Long-form detail for the three integrations with the most of it — Toolpath, transactional email, and cloud file storage. `MODULES.md` keeps each one's summary and the facts that are expensive to learn late (Toolpath speaks **millimetres**; email **blocks portal login**; the app owns the rev letter, not Dropbox). These pages are what you read when you actually build one. |
...
| `docs/WHEN-IT-GOES-WRONG.md` | `docs/` | **Read this one before you need it.** Written for the owner, not the AI: the warning signs that a stretch has gone sideways, the phrase to type when you are lost, the expensive rabbit holes named in order, what can and cannot be undone, and the secrets rule. The single habit it asks for is committing when something works. |
| `docs/GITHUB.md` | `docs/` | **Making a red build actually stop something.** GitHub blocks nothing by default: a failing check leaves the merge button green. Six settings, with both the click path and the `gh` command — required status checks, secret-scanning push protection, Dependabot, a read-only workflow token, a deploy environment that waits for a human, and when CODEOWNERS starts being worth it. These live in GitHub's settings, so no check in this repo can see them. |
| `.github/workflows/deploy.yml.template` + `docs/runbooks/Dockerfile.template` | root | **The deploy, gated.** A workflow that will not start unless the gates pass (`needs:`) and will not proceed without a human click (a `production` environment with a required reviewer) — then checks `/api/health` afterwards rather than trusting an exit code. Four target blocks: VPS + Compose (the default), ECS/Fargate (and GovCloud), Azure Container Apps, PaaS. Each honours its own one-shot migration construct (`OPS-2`). Shipped as `.template` so a half-configured deploy cannot fire on your first push. |
...
| File | Goes in | Purpose |
|---|---|---|
| `.gitignore` | repo root | Secrets/artifacts ignore list — verify it covers your stack **before the first commit**. Also ignores where **review, audit and screenshot output** lands (`reviews/`, `screenshots/`, `shots/`, `docs/reviews/`, `*.review.md`, `panel-review-*.md`) and scratch working files (`*.draft.md`, `*.bak`). Those state plainly where the product is weak, or show it — that candour is the point and is meant to stay local; the *fixes* are what reach the repo. Images are **not** blanket-ignored, so a logo referenced by `BRAND.md` still commits. |
